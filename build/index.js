...
import { z } from "zod";
import { PROPERTIES, OWNERS, searchProperties, searchByOwnerName, getOwnersForProperty, getOverdueProperties, getTransactions, } from "./mock-data.js";
import { getTenementsForAssessment, } from "./wa-tenements.js";
const server = new McpServer({
    name: "rates-assist",
    version: "0.1.0",
});
// ----- READ TOOLS -----
server.registerTool("search_property", {
    description: "Search properties by address fragment, suburb, or assessment number. Returns matching properties.",
    inputSchema: {
    ...
    const text = results
        .map((p) => `${p.assessmentNumber} — ${p.address}, ${p.suburb} ${p.postcode} | ${p.landUse} | balance $${p.balance.toFixed(2)}`)
        .join("\n");
    return { content: [{ type: "text", text: `Found ${results.length} match(es):\n${text}` }] };
});
server.registerTool("search_by_owner", {
...
server.registerTool("get_property_detail", {
    ...
    if (!p)
        return { content: [{ type: "text", text: `No property with assessment ${assessmentNumber}.` }] };
    const owners = getOwnersForProperty(p);
    const ownerLines = owners
        .map((o) => `  - ${o.name} | ${o.phone ?? "no phone"} | ${o.email ?? "no email"} | postal: ${o.postalAddress} | since ${o.ownerSince}`)
        .join("\n");
    const text = [
...
server.registerTool("list_overdue", {
    ...
    if (!overdue.length)
        return { content: [{ type: "text", text: "No properties currently overdue." }] };
    const total = overdue.reduce((s, p) => s + p.balance, 0);
    const text = `${overdue.length} properties overdue, total $${total.toFixed(2)}:\n\n` +
        overdue
            .map((p) => {
            const owners = getOwnersForProperty(p).map((o) => o.name).join(", ");
...
server.registerTool("draft_payment_reminder", {
    description: "Draft a personalised payment reminder for a property's owner. Returns the draft. Does NOT send.",
    inputSchema: {
        assessmentNumber: z.string(),
    ...
    const p = PROPERTIES.find((x) => x.assessmentNumber === assessmentNumber);
    if (!p)
        return { content: [{ type: "text", text: `No property ${assessmentNumber}.` }] };
    if (p.balance <= 0)
...
server.registerTool("draft_chase_all_overdue", {
    description: "Draft a personalised payment reminder for every overdue property. Returns batch preview. Does NOT send.",
    inputSchema: {
        tone: z.enum(["friendly", "firm", "final"]).default("friendly"),
    },
}, async ({ tone }) => {
    ...
    if (!overdue.length)
        return { content: [{ type: "text", text: "Nothing to chase." }] };
...
server.registerTool("update_owner_contact", {
    description: "Update an owner's phone and/or email. Returns confirmation preview. Does NOT commit until confirmed.",
    inputSchema: {
        ownerId: z.string(),
    ...
    if (!owner)
        return { content: [{ type: "text", text: `No owner ${ownerId}.` }] };
    const changes = [];
    if (newPhone)
        changes.push(`phone: ${owner.phone ?? "none"} → ${newPhone}`);
    if (newEmail)
        changes.push(`email: ${owner.email ?? "none"} → ${newEmail}`);
    if (!changes.length)
...
function classifyMismatch(landUse, tenements) {
    if (!tenements.length)
        return null;
...
function estimateUplift(annualRatesNow, severity) {
...
server.registerTool("find_mining_mismatches", {
    description: "Cross-reference rated properties against active WA mining tenements (DMIRS data) and surface candidates where the rating classification appears mis-aligned with the actual land use. Returns ranked list with confidence and estimated annual uplift.",
    inputSchema: {
        minSeverity: z
    ...
    for (const p of PROPERTIES) {
        ...
        if (!mismatch)
            continue;
        if (sevRank[mismatch.severity] < sevRank[minSeverity])
            continue;
...
server.registerTool("get_tenement_for_property", {
...
server.registerTool("generate_evidence_pack", {
    description: "Produce a council-grade reclassification evidence pack for a mining-mismatch candidate. Includes property record, tenement evidence, statutory basis, estimated uplift, and draft notice text.",
    inputSchema: {
        assessmentNumber: z.string(),
    },
}, async ({ assessmentNumber }) => {
