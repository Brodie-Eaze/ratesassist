...
import type { schemas, Tenement } from "@ratesassist/contract";
import {
  buildEvidencePack,
  findMismatches,
  recoveryStats,
...
function tenementTypeLabel(type: Tenement["type"]): string {
  switch (type) {
    case "M":
      return "Mining Lease";
    case "E":
...
export async function findMiningMismatchesHandler(
  input: schemas.ToolInputs["find_mining_mismatches"],
  ctx: RequestContext,
...
export async function generateEvidencePackHandler(
  input: schemas.ToolInputs["generate_evidence_pack"],
  ctx: RequestContext,
): Promise<schemas.ToolResult> {
...
export async function recoverySummaryHandler(
  input: schemas.ToolInputs["recovery_summary"],
  ctx: RequestContext,
): Promise<schemas.ToolResult> {
  const candidates = findMismatches(ctx.evaluationContext, {
    ...(input.council !== undefined ? { council: input.council } : {}),
...
export async function getTenementForPropertyHandler(
  input: schemas.ToolInputs["get_tenement_for_property"],
  ctx: RequestContext,
): Promise<schemas.ToolResult> {
  ...
  const lines = tenements
    .map((t) =>
      [
        ...
        `  Area: ${intAu(t.areaHectares)} ha | Producing: ${t.isProducing ? "yes" : "no"}${
          t.lastWorkProgramYear !== null
