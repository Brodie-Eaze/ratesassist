...
const KNOWN_TOOLS = new Set(Object.keys(inputs));
...
export async function dispatch(args) {
    ...
    if (!isToolName(toolName)) {
        return failure("invalid_input", `Unknown tool "${toolName}".`, context.correlationId);
    }
    ...
    if (!validatedResult.success) {
        return failure("internal_error", `handler "${toolName}" returned a result that did not match the contract: ${validatedResult.error.issues
            .map((i) => i.message)
            .join("; ")}`, context.correlationId);
    }
