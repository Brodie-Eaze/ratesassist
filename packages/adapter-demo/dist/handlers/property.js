...
export async function getPropertyDetailHandler(input, ctx) {
    ...
    const ownerLines = owners.length > 0
        ? owners
            .map((o) => `  - ${o.name} | ${o.phone ?? "no phone"} | ${o.email ?? "no email"} | postal: ${o.postalAddress} | since ${o.ownerSince}`)
            .join("\n")
        : "  (no owner records resolved)";
    const tenementLines = tenements.length > 0
        ? tenements
            .map((t) => `  - ${t.tenementId} | ${t.type}-class ${t.status} | ${t.commodity.join(", ")} | holder: ${t.holder}${t.isProducing ? " | producing" : ""}`)
            .join("\n")
        : "  (no intersecting tenements on file)";
    const lastPaymentSegment = property.lastPaymentDate !== null
        ? `${property.lastPaymentDate}${property.lastPaymentAmount !== null
...
export async function getTransactionHistoryHandler(input, ctx) {
    const property = ctx.store.getProperty(input.assessmentNumber);
    if (property === undefined) {
        return notFound(`No property with assessment number "${input.assessmentNumber}".`, ctx.correlationId);
    }
    ...
    const lines = txs
        .map((t) => `${t.date} | ${t.type.padEnd(18)} | ${aud(t.amount).padStart(12)} | ${t.reference} | bal ${aud(t.balance)}`)
        .join("\n");
    return {
        ok: true,
...
export async function listPropertiesHandler(input, ctx) {
    ...
    const text = [
        ...
        ...page.map((p) => `  - ${p.assessmentNumber} | ${p.address}, ${p.suburb} | ${p.landUse} | balance ${aud(p.balance)}`),
    ].join("\n");
    return {
        ok: true,
        output: text,
