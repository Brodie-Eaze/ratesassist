 ...
 */
import { conflict, forbidden, invalidInput, notFound, } from "../runtime/errors.js";
...
export async function updateOwnerContactHandler(input, ctx) {
    ...
    if (input.confirm) {
        if (input.commitToken === undefined) {
            return invalidInput("confirm=true requires a commitToken from the preview call.", ctx.correlationId);
        }
        const consumed = ctx.commitTokens.consume(input.commitToken, "update_owner_contact");
        if (!consumed.ok) {
            const reason = consumed.reason === "expired"
        ...
        if (mut.ownerId !== input.ownerId) {
            return conflict("commitToken was issued for a different ownerId.", ctx.correlationId);
        }
        const updated = {
            ...owner,
        ...
        if (stored === undefined) {
            ...
            return notFound(`Owner "${input.ownerId}" no longer exists.`, ctx.correlationId);
        }
        return {
            ok: true,
    ...
    const { lines, hasChanges, patch } = diffOwnerContact(owner, input.newPhone, input.newEmail);
    if (!hasChanges) {
        return {
            ok: true,
...
export async function addPropertyNoteHandler(input, ctx) {
    ...
    if (input.confirm) {
        ...
        const consumed = ctx.commitTokens.consume(input.commitToken, "add_property_note");
        if (!consumed.ok) {
            const reason = consumed.reason === "expired"
                ? "commitToken has expired (5 minute TTL); re-run the preview"
                : consumed.reason === "operation_mismatch"
        ...
        if (mut.assessmentNumber !== input.assessmentNumber) {
            return conflict("commitToken was issued for a different assessment number.", ctx.correlationId);
        }
        const stored = ctx.store.addNoteToProperty(input.assessmentNumber, mut.note);
        if (stored === undefined) {
            return notFound(`Property "${input.assessmentNumber}" no longer exists.`, ctx.correlationId);
        }
        return {
            ok: true,
...
const CERTIFICATE_TYPES_BY_STATE = {
    WA: ["WA-6.76", "WA-S6.76"],
...
— Issued by the council under delegated authority.`;
    switch (state) {
        ...
        case "QLD":
            return qld;
        default:
            throw new Error(`certificateBodyFor invoked for unsupported state ${state}`);
    }
}
/** `generate_statutory_certificate` handler. */
export async function generateStatutoryCertificateHandler(input, ctx) {
    const property = ctx.store.getProperty(input.assessmentNumber);
    if (property === undefined) {
        return notFound(`No property with assessment number "${input.assessmentNumber}".`, ctx.correlationId);
    }
    ...
    if (expected === undefined || expected.length === 0) {
        return forbidden(`Statutory certificate generation for ${property.state} is not yet supported by this adapter (see README for the deferred-state list).`, ctx.correlationId);
    }
    if (!expected.includes(input.certificateType)) {
        return invalidInput(`certificateType "${input.certificateType}" is not valid for ${property.state}; expected one of: ${expected.join(", ")}.`, ctx.correlationId);
    }
