 ...
 */
function composeDraft(property, owner, tone) {
    const subject = tone === "final"
...
export async function draftPaymentReminderHandler(input, ctx) {
...
export async function draftChaseAllOverdueHandler(input, ctx) {
    ...
    const summaryLines = drafts
        .map((d) => `  - ${d.assessmentNumber} → ${d.recipient} | ${d.recipientPhone ?? "no phone"} / ${d.recipientEmail ?? "no email"}`)
        .join("\n");
    const text = [
