...
export async function GET(
  ...
  // file is like "TPS-1102-44.md" or "TPS-1102-44.html"
  const dot = file.lastIndexOf(".");
  if (dot === -1) {
    return NextResponse.json({ error: "missing extension" }, { status: 400 });
  }
  const assessment = file.slice(0, dot);
  const ext = file.slice(dot + 1);
  const pack = buildEvidencePack(assessment);
  if (!pack) {
    return NextResponse.json({ error: "no pack" }, { status: 404 });
  }
