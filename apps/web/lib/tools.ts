...
import { fetchDmirsTenementsForCouncil } from "@ratesassist/spatial";
import {
  COUNCILS,
  getAllLiveTenements,
...
const HANDLERS: Handlers = {
  ...
  async generate_evidence_pack({ assessmentNumber }) {
    const ctx = getEvaluationContext();
    const result = buildEvidencePack(assessmentNumber, ctx);
    switch (result.kind) {
      case "ok":
        return result.pack.markdown;
      case "no_property":
        return `No evidence pack generated: assessment ${assessmentNumber} not found in the rating register.`;
      case "no_signals":
        return `No evidence pack generated: ${assessmentNumber} (${result.property.address}, ${result.property.suburb}) has no detection signals firing — nothing to recover.`;
      case "no_owner":
        return `No evidence pack generated: ${assessmentNumber} has signals firing but the rated owner could not be resolved. Reconcile the rating system before drafting a notice.`;
    }
  },

  async recovery_summary({ council }) {
    const stats = recoveryStatsForWeb(council);
    return [
  ...
  async verify_abn({ abn }) {
    const result = await abnClient.lookupAbn(abn);
  ...
  async fetch_dmirs_tenements({ council }) {
    // Seed the DMIRS probe with our in-process tenement set as GeoJSON
    // features. The new package only labels a result `"live"` when real
    // GetFeature data has been parsed; otherwise it returns `"seeded"` with
    // the caller-supplied features. That honest labelling is the contract.
    const seededFeatures = getAllLiveTenements().map((t) => ({
      ...
      geometry: {
        ...
        // Convert Leaflet [lat, lng] order to GeoJSON [lng, lat] order.
        coordinates: [t.polygon.map(([lat, lng]) => [lng, lat])],
      },
    }));
    ...
    const previewCount = Math.min(5, result.features.length);
    const sampleLines = result.features
      .slice(0, previewCount)
      .map((f) => {
        const props = f.properties as Record<string, unknown>;
        const tenementId = String(props["tenementId"] ?? "tenement");
        const status = String(props["status"] ?? "unknown");
        const commodity = String(props["commodity"] ?? "");
        return `- ${tenementId} (${status}${commodity ? `, ${commodity}` : ""})`;
      })
      .join("\n");

    const lines = [
      `Fetched **${result.features.length}** tenement(s) intersecting ${council} (source: ${result.source}).`,
    ];
    if (result.note !== undefined) lines.push(`_${result.note}_`);
    if (sampleLines.length > 0) lines.push("", "Sample:", sampleLines);
    return lines.join("\n");
  },
