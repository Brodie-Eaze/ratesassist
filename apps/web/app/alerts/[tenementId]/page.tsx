      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-ink-50">
        ...
        <div className="px-6 py-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 card overflow-hidden">
            ...
            {intersectingParcels.length === 0 ? (
              <div className="p-6 text-sm text-ink-600">
                No council-registered parcels intersect this tenement
                geometry. The tenement may sit over Crown land, pastoral
                lease, or unrated parcels.
              </div>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {intersectingParcels.map((p) => (
                    <tr
                      key={p.assessmentNumber}
                      className={`border-t border-ink-200 ${
                        highlightedParcel === p.assessmentNumber
                          ? "bg-accent-50"
                          : "hover:bg-ink-50"
                      }`}
                    >
