...
function Recenter({
  ...
  centre: [number, number];
  zoom: number;
  fitProperties?: readonly Property[];
}) {
  const map = useMap();
...
function FitToTenement({ tenement }: { tenement?: Tenement }) {
  ...
  useEffect(() => {
    if (!tenement) return;
    const bounds = L.latLngBounds(tenement.polygon.map((pt) => [pt[0], pt[1]] as [number, number]));
...
export default function MapInner({
  ...
  return (
    ...
    >
      ...
      {!liveVectors &&
        tenements.map((t) => {
          ...
          return (
            <Polygon
              key={t.tenementId}
              positions={t.polygon.map((pt) => [pt[0], pt[1]] as [number, number])}
              pathOptions={{
      ...
      {selectedTenement && (() => {
        const centroid = L.latLngBounds(
          selectedTenement.polygon.map((pt) => [pt[0], pt[1]] as [number, number]),
        ).getCenter();
        return (
        <Polygon
          positions={bufferPolygon(centroid.lat, centroid.lng, 1000).map(
